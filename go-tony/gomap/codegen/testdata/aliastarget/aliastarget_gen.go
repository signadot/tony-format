...
// ToTonyIR converts Real to a Tony IR node.
func (s *Real) ToTonyIR(opts ...gomap.MapOption) (*ir.Node, error) {
	if s == nil {
		return ir.Null(), nil
	}
...
// FromTonyIR populates Real from a Tony IR node.
func (s *Real) FromTonyIR(node *ir.Node, opts ...gomap.UnmapOption) error {
	if node == nil {
		return nil
	}
	...
	if node.Type != ir.ObjectType {
		return fmt.Errorf("expected map for Real, got %v", node.Type)
	}
...
// ToTony converts Real to Tony format bytes.
func (s *Real) ToTony(opts ...gomap.MapOption) ([]byte, error) {
...
// FromTony parses Tony format bytes and populates Real.
func (s *Real) FromTony(data []byte, opts ...gomap.UnmapOption) error {
