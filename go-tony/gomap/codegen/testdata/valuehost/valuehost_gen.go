...
func (s *Host) ToTonyIR(opts ...gomap.MapOption) (*ir.Node, error) {
	...
	// Field: Val
	node, err = s.Val.ToTonyIR(opts...)
	if err != nil {
		return nil, fmt.Errorf("failed to convert field %q: %w", "Val", err)
	}
	irMap["val"] = node
...
func (s *Host) FromTonyIR(node *ir.Node, opts ...gomap.UnmapOption) error {
	...
	for i, fieldName := range node.Fields {
		...
		case "val":
			// Field: Val
			if err := s.Val.FromTonyIR(fieldNode, opts...); err != nil {
				return fmt.Errorf("field %q: %w", "val", err)
			}
		}
	}
