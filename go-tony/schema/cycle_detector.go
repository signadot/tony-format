...
//
// # Desired Approach: SAT-based Satisfiability
//
...
//
// The current implementation uses heuristics (inArray, inNullable, isOptional flags)
// propagated during tree traversal, rather than SAT evaluation. This misses cases like:
//
//   - !and [!not null, .node]: flags see null and incorrectly mark as escapable
//   - Contradictory non-recursive constraints are not checked
//   - Untagged arrays [...] are not treated as implicit AND
//   - Boolean operator nesting (!not.or, !not.and) is not correctly evaluated
//
// TODO: Replace heuristic flag propagation with SAT-based formula evaluation
// using the gini solver (already imported).
//   - Build formula from accept: field with inline expansion of references
//   - Substitute self-references with constant false
//   - Check satisfiability (UNSAT → error)
