...
func TestWatchHub_SubscribeUnsubscribe(t *testing.T) {
	...
	// Subscribe
	sub1 := NewWatcher("users", nil, true, 10)
	hub.Watch(sub1)
	...
	sub2 := NewWatcher("users", nil, false, 10)
	hub.Watch(sub2)
	...
	sub3 := NewWatcher("posts", nil, true, 10)
	hub.Watch(sub3)
...
func TestWatchHub_Broadcast_ExactMatch(t *testing.T) {
	...
	sub := NewWatcher("users", nil, true, 10)
	hub.Watch(sub)
...
func TestWatchHub_Broadcast_ParentMatch(t *testing.T) {
	...
	sub := NewWatcher("users.alice", nil, true, 10)
	hub.Watch(sub)
	defer hub.Unwatch(sub)
...
func TestWatchHub_Broadcast_SlowConsumerFails(t *testing.T) {
	...
	sub := NewWatcher("users", nil, true, 1)
	hub.Watch(sub)
...
func TestWatchHub_Concurrent(t *testing.T) {
	...
	for i := 0; i < numGoroutines; i++ {
		...
		go func(id int) {
			defer wg.Done()
			for j := 0; j < numOps; j++ {
				sub := NewWatcher("test", nil, true, 10)
				hub.Watch(sub)
				hub.Unwatch(sub)
			}
		}(i)
