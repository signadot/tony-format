...
func TestReadTxLog_BinarySearch(t *testing.T) {
	...
	entries := []*TxLogEntry{
		{Commit: 1, TxID: 1, Timestamp: "2024-01-01T00:00:00Z", PendingFiles: []FileRef{{VirtualPath: "/a", TxSeq: 1}}},
		{Commit: 3, TxID: 2, Timestamp: "2024-01-01T00:01:00Z", PendingFiles: []FileRef{{VirtualPath: "/b", TxSeq: 2}}},
		{Commit: 5, TxID: 3, Timestamp: "2024-01-01T00:02:00Z", PendingFiles: []FileRef{{VirtualPath: "/c", TxSeq: 3}}},
		{Commit: 7, TxID: 4, Timestamp: "2024-01-01T00:03:00Z", PendingFiles: []FileRef{{VirtualPath: "/d", TxSeq: 4}}},
		{Commit: 10, TxID: 5, Timestamp: "2024-01-01T00:04:00Z", PendingFiles: []FileRef{{VirtualPath: "/e", TxSeq: 5}}},
		{Commit: 15, TxID: 6, Timestamp: "2024-01-01T00:05:00Z", PendingFiles: []FileRef{{VirtualPath: "/f", TxSeq: 6}}},
		{Commit: 20, TxID: 7, Timestamp: "2024-01-01T00:06:00Z", PendingFiles: []FileRef{{VirtualPath: "/g", TxSeq: 7}}},
	}

	// Write all entries
	for _, entry := range entries {
		if err := storage.AppendTxLog(entry); err != nil {
			t.Fatalf("failed to append log entry: %v", err)
		}
	}

	// Test 1: Read all entries (nil parameter)
	allEntries, err := storage.ReadTxLog(nil)
	if err != nil {
	...
	for i, expected := range entries {
		...
		if allEntries[i].TxID != expected.TxID {
			t.Errorf("entry %d: expected txID %d, got %d", i, expected.TxID, allEntries[i].TxID)
		}
	}

	// Test 2: Binary search for commitCount >= 5
	minCommit := int64(5)
	filteredEntries, err := storage.ReadTxLog(&minCommit)
	if err != nil {
...
func TestReadTxLog_EmptyLog(t *testing.T) {
	// Create temporary directory
	tmpDir, err := os.MkdirTemp("", "logd-test-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	...
	entries, err := storage.ReadTxLog(nil)
	if err != nil {
	...
	minCommit := int64(5)
	entries, err = storage.ReadTxLog(&minCommit)
	if err != nil {
...
func TestReadTxLog_NonExistentFile(t *testing.T) {
	// Create temporary directory
	tmpDir, err := os.MkdirTemp("", "logd-test-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
