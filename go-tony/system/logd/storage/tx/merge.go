...
func (kt *kTree) node() (*ir.Node, error) {
	...
	case keyedArrayKind:
		// A (key) segment names an element by its key VALUE. The store names an element
		// by its identity, as a field of the object that holds it -- items."(sku=A)" --
		// and a patch is rooted at that field like any other (element_identity.md).
		return nil, fmt.Errorf("cannot root a patch at a (key) segment: an element is " +
			"addressed by its identity, as the field (field=value) of the array that holds it")
	case objectKind:
