...
// RootPatchAt cannot take a (key) segment: `items("A")` names an element by value, and
// the store names it by identity, as a field of the array that holds it -- the path a
// client's sugar canonicalizes to before it gets here (element_identity.md).
func TestRootPatchAtRefusesAKeySegment(t *testing.T) {
	_, err := RootPatchAt(`items("A")`, ir.FromMap(map[string]*ir.Node{"qty": ir.FromInt(5)}))
	if err == nil {
		t.Fatal("a (key) segment was accepted")
	}
	if !strings.Contains(err.Error(), "identity") {
		t.Errorf("error %q does not say how an element is addressed", err)
	}
	// The field form is an ordinary path.
	rooted, err := RootPatchAt(`items."(sku=A)"`, ir.FromMap(map[string]*ir.Node{"qty": ir.FromInt(5)}))
	if err != nil {
		t.Fatalf("the field form was refused: %v", err)
	}
	if at, err := rooted.GetKPath(`items."(sku=A)".qty`); err != nil || at == nil {
		t.Errorf("not rooted at the name: %s", encode.MustString(rooted))
	}
}
