...
// NextCommitIfPathSeqMatches conditionally increments the commit count
...
func (s *Seq) NextCommitIfPathSeqMatches(
	path string,
	expectedSeq *int64,
) (int64, bool, error) {
	s.Lock()
	...
	state.PathSeqs[path] = state.Commit
...
func (s *Seq) ReadStateLocked() (*State, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			return &State{Commit: 0, TxSeq: 0, PathSeqs: make(map[string]int64)}, nil
		}
		return nil, err
	}
