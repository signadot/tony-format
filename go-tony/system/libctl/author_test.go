...
func authorOf(t *testing.T, logdAddr string, commit int64) string {
	...
	for {
		...
		case ev := <-w.Events():
			if ev == nil {
				t.Fatalf("the watch ended before commit %d was replayed; saw %v", commit, seen)
			}
			seen = append(seen, fmt.Sprintf("{commit %d state %v patch %v author %q end %s}", ev.Commit, ev.State != nil, ev.Patch != nil, ev.Author, ev.EndReason))
			if ev.Patch != nil && ev.Commit == commit {
				return ev.Author
			}
		case <-deadline:
			t.Fatalf("commit %d was not replayed; saw %v", commit, seen)
		}
	}
}
...
// write leaves the client's session -- onto the routed patch, or onto the transaction a
// split write becomes -- so no hop commits it under a hello that is not the client's
// (cn1n32yph12ks5wrmhn0).
func TestAuthorRidesEveryRouteThroughDocd(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "split, the patch's own", path: "",
			...
			opts: PatchOpts{Author: "carol"}, want: "carol"},
	}
...
// A client's own transaction through docd is written by the author its newtx named
// (else the session's), and every participant inherits it across the controller hop: a
// participant names none, and one that does is refused as it would be on logd.
func TestClientTransactionThroughDocdHasNewtxsAuthor(t *testing.T) {
	logd := startLogd(t)
	docd := startDocdRouting(t, logd.TCPAddr())
	...
	for i, tc := range []struct {
		...
		{name: "the session's", want: "verse"},
		{name: "newtx's own", opts: NewTxOpts{Author: "alice"}, want: "alice"},
	} {
		// Distinct data per transaction: a write that changes nothing at the path is a
		// commit the watch has nothing to say about.
		data := vObj(int64(i + 1))
		t.Run(tc.name, func(t *testing.T) {
			txID, err := client.NewTxWith(ctx, 2, tc.opts)
			if err != nil {
				t.Fatalf("newtx through docd: %s", err)
			}
			if _, err := client.PatchWith(ctx, "verse.b.y", vObj(0), PatchOpts{TxID: &txID, Author: "bob"}); api.ErrorCode(err) != api.ErrCodeInvalidTx {
				t.Fatalf("a participant naming an author: err = %v, want %s", err, api.ErrCodeInvalidTx)
			}
			type res struct {
				r   *api.PatchResult
				err error
			...
			for _, p := range []string{"verse.a.x", "verse.b.y"} {
				go func(p string) {
					r, err := client.PatchTx(ctx, p, data, txID)
					done <- res{r, err}
				}(p)
			}
			var commit int64
			...
			if got := authorOf(t, logd.TCPAddr(), commit); got != tc.want {
				t.Errorf("the transaction is recorded as written by %q, want %q", got, tc.want)
			}
		})
	}
}
