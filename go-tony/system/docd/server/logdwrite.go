...
func writeBaseParticipant(logdAddr string, txID int64, path string, base, match *ir.Node, matchPath string, timeout time.Duration) (*logdapi.SessionResponse, error) {
	...
	if err := writeSessionRequest(conn, &logdapi.SessionRequest{
		Hello: &logdapi.Hello{ClientID: "docd-base"},
	}); err != nil {
