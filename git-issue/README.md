...
which puts `git-issue` in `$(go env GOBIN)` (or `$GOPATH/bin`); or, from a
checkout, `go build .` in this directory and move the binary anywhere on PATH.
...
git-issue is a Go module of its own beside go-tony, which it imports by version,
so it releases (`git-issue/vX.Y.Z`) when it changes rather than when the library
does.
