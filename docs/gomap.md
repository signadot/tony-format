...
There are Tony [schema](/docs/tonyschema/index.md) and [IR](/docs/ir.md) coordinating
things.
...
> [!WARNING]
> **Compatibility Note**: Doc comment directives (`//tony:`) are only supported by the `tony-codegen` tool. They are **NOT** visible to the runtime reflection-based `gomap.ToTonyIR()`/`gomap.FromTonyIR()` and `gomap.ToTony()`/`gomap.FromTony()` functions, because Go reflection cannot access comments at runtime. If you use doc comment directives, you **MUST** use code generation (`ToTony`/`FromTony` methods). For reflection-based usage, you must use struct tags.
...
#### `ToTony(opts ...gomap.MapOption) ([]byte, error)` - Primary API

Serializes the struct to Tony format bytes:

```go
func (s *Person) ToTony(opts ...gomap.MapOption) ([]byte, error) {
...
#### `FromTony(data []byte, opts ...gomap.UnmapOption) error` - Primary API

Deserializes Tony format bytes into the struct:

```go
func (s *Person) FromTony(data []byte, opts ...gomap.UnmapOption) error {
...
#### `ToTonyIR(opts ...gomap.MapOption) (*ir.Node, error)` - Advanced

Serializes the struct to a Tony IR node (for advanced IR manipulation):

```go
func (s *Person) ToTonyIR(opts ...gomap.MapOption) (*ir.Node, error) {
...
#### `FromTonyIR(node *ir.Node, opts ...gomap.UnmapOption) error` - Advanced

Deserializes a Tony IR node into the struct (for advanced IR manipulation):

```go
func (s *Person) FromTonyIR(node *ir.Node, opts ...gomap.UnmapOption) error {
...
// Read from file
data, _ := os.ReadFile("user.tony")
node, _ = parse.Parse(data)
